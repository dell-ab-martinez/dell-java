...
public class Controller {
    ...
    public void processListAction(String[] actionParts){
        if(actionParts.length > 3){
        	...
        	for(int i = 1; i < actionParts.length; i++) {
        		if (actionParts[i].equals("-a")) {
        			activeOnly = true;
        		} else {
        			projectName = actionParts[i];
        		}
        	}
        	
        	List<TimesheetEntry> database = timesheet.list(activeOnly, projectName);
            consoleUtils.printList(database);
        }
    ...
    public void processAddAction(){
    	// Collect project name and task
    	try {
    		String project = consoleUtils.promptString("Project Name (one word only):");
    		String[] projectNameParts = project.split(" ");
    		
    		if (projectNameParts.length > 1) {
    			throw new IllegalArgumentException("Project name can be one word only.");
    		} else {
